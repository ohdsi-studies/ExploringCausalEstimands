...
ggplot(vizData, aes(x = x, y = y, color = label, fill = label, group = label)) +
  ...
  geom_line(aes(linetype = label), linewidth = 0.75, alpha = 0.7) +
...
ggsave(filename = "Simulations/plots/HrsAndRrsOutcomeSusceptibles.pdf", width = 7, height = 4)
